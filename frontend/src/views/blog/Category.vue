...
async function loadCategory() {
  ...
  try {
    ...
    category.value = categories.find(c => c.slug === route.params.slug) || null

    if (category.value) {
      const { data } = await postApi.list({ category_id: category.value.id })
...
<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div v-else class="text-center py-20">
      <p class="text-slate-500 dark:text-slate-400">分类不存在</p>
